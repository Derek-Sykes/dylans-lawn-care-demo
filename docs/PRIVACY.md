...
The installation operator and invited owners sign in with their own approved Google accounts to use one shared workspace. Only the operator can issue or revoke invitations and remove invited owners' access. The application stores each approved account's stable identifier, email address and access status. Joining does not connect a calendar. An approved person can separately connect Google Calendar when the workspace has none assigned. The application checks busy times in that account's primary calendar and a dedicated booking calendar, and creates or removes appointments in that dedicated calendar. It does not request access to edit personal events in other calendars.

The assigned Calendar account holder can separately authorize send-only Gmail permission for appointment receipts, confirmations, updates, cancellations and reminders. Portal sign-in and Calendar consent do not grant Gmail access. The application does not read inbox messages, inspect replies or bounces, or request inbox-reading scopes. Automated messages go to the customer's supplied email address and include appointment details. Private admin notes and customer freeform notes are excluded. Gmail receives the outgoing message and recipient; replies go to the connected Gmail account. See [appointment email behavior](EMAIL_NOTIFICATIONS.md).
...
Owners can turn off automatic appointment emails. The assigned Calendar account holder can also disconnect only the email sender, removing this installation's stored Gmail grant without revoking Calendar access. Pending automatic messages are skipped; a message already submitted to Gmail cannot be withdrawn this way.
