...
The assigned Calendar account's send-only Gmail permission supports appointment receipts, confirmations, updates, cancellations and reminders. Email automation remains disabled until an owner enables it. Existing Calendar-only connections require an additional consent through **Emails → Enable Gmail** to add sending permission. **Reconnect Google**, available when Calendar needs attention, requests Calendar and sending permissions together. The application does not read inbox messages, inspect replies or bounces, or request inbox-reading scopes. Automated messages go to the customer's supplied email address and include appointment details. Private admin notes and customer freeform notes are excluded. Gmail receives the outgoing message and recipient; replies go to the connected Gmail account. See [appointment email behavior](EMAIL_NOTIFICATIONS.md).
