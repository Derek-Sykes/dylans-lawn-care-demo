...
An unclaimed installation requires its private setup link before Google can claim the owner account. An operator can open that link securely using the server's bootstrap token in the URL fragment; do not publish or paste the link into tickets or logs. After ownership is established, the owner can return to `/admin/` and use Google sign-in. Google may require reauthorization if consent is revoked or its token expires; an application update itself does not disconnect the account.
