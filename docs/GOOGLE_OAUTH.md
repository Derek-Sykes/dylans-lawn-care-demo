...
Appointments appear in a separate booking calendar within the owner's Google Calendar. Availability checks include the owner's primary calendar and the dedicated booking calendar. Other secondary calendars are not included in this first version. The application cannot edit personal events in the primary calendar. Portal sign-in and Calendar connection do not request Gmail permissions.

Appointment email has its own explicit consent step in **Emails**, available to the assigned Calendar account holder. It requests `openid email` plus `https://www.googleapis.com/auth/gmail.send` and verifies that the signed-in Google identity is that same Calendar account. The Gmail grant is saved separately from the Calendar grant. This permission allows sending email; it does not allow reading the inbox, inspecting received messages or accessing Gmail profile/history APIs. Other workspace owners can manage appointments without granting access to their own Gmail. See [appointment email behavior](EMAIL_NOTIFICATIONS.md) and [Google's Gmail scope definitions](https://developers.google.com/workspace/gmail/api/auth/scopes).
